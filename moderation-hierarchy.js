/* Creator / Right Hand staff administration. All decisions are checked again by server RPCs. */
(() => {
  'use strict';
  ...
  const base = 'https://znenamrszhjsiztllcit.supabase.co';
  const db = window.supabase.createClient(base, 'sb_publishable_3VRFxwtDuYq4ETHs4xof8g_Fp3GRl6c',
  ...
  for(const [value] of [['moderator'],['right_hand']]) { const opt=elem('option');opt.value=value;role.append(opt); }
  const roleLabel=elem('label');roleLabel.append(role);
  const send=elem('button','community-action');send.type='submit';
  form.append(usernameLabel,roleLabel,send);
  ...
  async function rpc(name,args){const response=await db.rpc(name,args);if(response.error)throw response.error;return response.data;}
  ...
  function sync(){
    ...
    usernameLabel.firstChild?.nodeType; // Labels are rebuilt text-only without removing their inputs.
    usernameLabel.childNodes.forEach(n=>{if(n.nodeType===3)n.remove();});
    usernameLabel.prepend(document.createTextNode(t('Usuário (sem @): ','Username (without @): ')));
    roleLabel.childNodes.forEach(n=>{if(n.nodeType===3)n.remove();});
    roleLabel.prepend(document.createTextNode(t('Cargo: ','Role: ')));
  ...
  async function load(){
    ...
    say(t('Carregando equipe…','Loading staff…'));
    try{
      ...
      for(const person of people||[]){
        ...
        li.append(elem('strong','',`${name} · ${person.role==='creator'?t('Criador','Creator'):person.role==='right_hand'?t('Braço Direito','Right Hand'):t('Moderador normal','Normal moderator')}`));
        const actions=elem('div','community-actions');
        if(ownRole==='creator'&&person.role!=='creator'){
          const flip=person.role==='right_hand'?'moderator':'right_hand';
          actions.append(button(flip==='right_hand'?'Promover':'Rebaixar',flip==='right_hand'?'Promote':'Demote',async()=>{
            ...
            await rpc('staff_assign',{p_user_id:person.user_id,p_role:flip,p_reason:why});
        ...
        if(person.role==='moderator'||ownRole==='creator'){
          ...
          actions.append(button(ownRole==='creator'?'Banir moderador':'Solicitar banimento',
            ownRole==='creator'?'Ban staff':'Request ban',async()=>{
              ...
              if(ownRole==='creator'){
                const days=window.prompt(t('Duração em dias (0 = permanente, máximo 3650):','Duration in days (0 = permanent, max 3650):'),'7');
        ...
        if(ownRole==='creator'&&person.role!=='creator')actions.append(button('Desbanir (se necessário)','Unban (if needed)',async()=>{
      ...
      if(ownRole==='creator'){
        ...
        for(const req of pending||[]){
          const li=elem('li','comment-item');
          const kind={grant_normal:t('Nomear moderador','Appoint moderator'),remove_normal:t('Remover moderador','Remove moderator'),ban_normal:t('Banir moderador','Ban moderator')}[req.kind];
        ...
        for(const item of review||[]){
          const li=elem('li','comment-item');li.append(elem('strong','',item.display_name||item.user_id),
  ...
  form.addEventListener('submit',async event=>{
    ...
    try{
      ...
      username.value='';say(ownRole==='creator'?t('Permissão concedida.','Role granted.'):t('Solicitação enviada ao Criador.','Request sent to Creator.'));await load();
