-- Applied on 2026-09-21 as migration fanart_submissions_private_queue.
-- Private submissions only: nothing is published automatically.
...
begin
  if new.user_id is distinct from auth.uid() or public.is_banned() or
     not exists (select 1 from auth.users u where u.id = new.user_id and u.email_confirmed_at is not null and u.is_anonymous is false)
  then
...
-- No SELECT/UPDATE/DELETE policy on the private bucket for visitors.
