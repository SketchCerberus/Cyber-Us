...
-- Spoiler protection in the UI is a reader choice, not private file storage after publication.
...
-- Report authors are never exposed to visitors. Report records disappear when the
-- associated comment is removed; the separate comment moderation log remains private.
...
GRANT INSERT(comment_id,reporter_id,reason) ON public.fanart_comment_reports TO authenticated;
GRANT SELECT(id,comment_id,reason,status,created_at) ON public.fanart_comment_reports TO authenticated;
GRANT UPDATE(status,handled_at) ON public.fanart_comment_reports TO authenticated;
...
CREATE POLICY fanart_report_user_insert ON public.fanart_comment_reports
  ...
  WITH CHECK (reporter_id = (SELECT auth.uid()) AND status = 'open' AND handled_at IS NULL
...
-- A trigger checks identity and rate limits without granting SELECT on private
-- comment author IDs or reporter records to ordinary authenticated accounts.
